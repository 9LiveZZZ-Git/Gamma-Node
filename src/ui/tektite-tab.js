...
async function _tektiteNavigateWikilink(target) {
  ...
  // Phase C §5.1 -- resolve beyond Tektite notes, in priority order:
  // .gdsp user types -> built-in registry node types -> demo patches.
  // First hit wins; nothing here mutates the patch except loadDemo(),
  // which confirms before replacing.

  // 3a. User DSP (.gdsp) type by name -> open it in the User DSP editor.
  if (USER_DSP_SOURCES && typeof USER_DSP_SOURCES === "object") {
    const dspKey = Object.keys(USER_DSP_SOURCES).find(k => k.toLowerCase() === targetLc);
    if (dspKey) {
      if (typeof switchTab === "function") switchTab("udsp");
      try { editingUdsp = dspKey; } catch (_) {}
      if (typeof setUdspText === "function") setUdspText(USER_DSP_SOURCES[dspKey] || "");
      if (typeof renderUdspList === "function") renderUdspList();
      return;
    }
  }

  // 3b. Built-in registry node type by name -> surface it in the Nodes
  // palette (does not add it to the patch).
  if (typeof TYPES === "object" && TYPES) {
    const typeKey = Object.keys(TYPES).find(k => k.toLowerCase() === targetLc);
    if (typeKey) {
      const nodesTab = document.getElementById("br-tab-nodes");
      if (nodesTab) nodesTab.click();
      const searchEl = document.getElementById("search");
      if (searchEl) searchEl.value = typeKey;
      if (typeof renderPalette === "function") renderPalette(typeKey);
      return;
    }
  }

  // 3c. Demo patch by id or name -> loadDemo() confirms before replacing.
  if (Array.isArray(_demos)) {
    const demoHit = _demos.find(d =>
      ((d && d.id)   || "").toLowerCase() === targetLc ||
      ((d && d.name) || "").toLowerCase() === targetLc);
    if (demoHit && typeof loadDemo === "function") {
      loadDemo(demoHit.id);
      return;
    }
  }

  // 4. Nothing matched anywhere -- offer to create a note. Vault only.
  if (tektiteSourceIsWritable("vault") &&
      window.confirm("No note, .gdsp type, node, or demo named \"" + target + "\" was found.\n\nCreate a note named \"" + target + "\" in the Vault?")) {
    try {
