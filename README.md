...
[![Version](https://img.shields.io/badge/version-0.3.686-green.svg)](VERSION)
...
| File | What it is |
|------|------------|
| `gamma-node-editor.html` | The editor. Single self-contained HTML file (HTML + inline CSS + vanilla JavaScript, no framework, no runtime dependencies). **Build output** — generated by `node build.mjs` from the `src/` tree. Ship this file. |
| `src/` | The source tree. ~99k lines of vanilla JS across 90 files in 14 subsystem directories (`core/`, `nodes/`, `codegen/`, `visual/`, `audio/`, `ai/`, `ui/`, `rig/`, `physics/`, `planet/`, `hwr/`, `persistence/`, `assets/`, `demos/`). Plus `src/styles/app.css` and `src/shell.html`. |
| `build.mjs` | Zero-dependency Node script (~30 lines). Reads `src/build-order.json`, concatenates every `.js` file in source order into one `<script>` block, inlines the CSS, fills the `APP_VERSION` token, writes `gamma-node-editor.html`. No `node_modules`, no bundler. |
...
```bash
# Edit any file under src/, then:
node build.mjs        # regenerates gamma-node-editor.html
```

The build is deterministic: same source bytes → same output bytes. No transpiling, no minification, no source maps to manage. Open `gamma-node-editor.html` directly to test.

The single-emailable-file property is preserved because the build inlines everything (JS + CSS + version) into one self-contained HTML — but you get a navigable `src/` tree to develop in.
